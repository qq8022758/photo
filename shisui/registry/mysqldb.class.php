<?php
/**
 * Database management use mysqli package
 * 
 * @author Ye Chen
 * @version 1.0
 */
class Mysqldb {
     ...
     * @param String the database table
     * @param array data to insert field => value
	* @param condition example:where actionid='$actid' and userid='$userid'
     * @return bool
     */
    public function insertRecordsByCondition( $table, $data,$condition ){
    	// setup some variables for fields and values
    	$fields  = "";
		...
		$insert = "INSERT INTO $table ({$fields}) VALUES({$values}) where (select count(*) from $table where $condition)<=0";
	...
	
    {
		...
		{
			$this->affected_rows = $this->connections[$this->activeConnection]->affected_rows;
			$this->last = $result;
		}
    ...
    {
    	return $this->last->fetch_all($model);
    }    
