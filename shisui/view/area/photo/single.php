...
<script>
$(".fangtude").append("<iframe width='960' id='singleid' height='800' scrolling='no'  src='view/area/photo/singleI.php?id=<?php echo $_GET['id']?>&uid="+config.user.id+"&friend=<?php echo isset($_GET['friend'])?$_GET['friend']:'0'?>&username="+config.user.username+"&email="+config.user.mail_address+"&cid="+config.user.cat_id+"'/><div class='geiiframeyongde'></div><div class='qufenxiangde'></div>");
$(".geiiframeyongde").unbind('click').bind('click',function(){
	...
	$(".qufenxiangde").unbind('click').bind('click',function(){
